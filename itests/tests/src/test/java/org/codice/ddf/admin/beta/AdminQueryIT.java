...
import org.codice.ddf.admin.beta.request.LdapRequestHelper;
import org.codice.ddf.admin.beta.request.WcpmRequestHelper;
...
public class AdminQueryIT extends AbstractComponentTest {
    @Override
    public List<Option> customSettings() {
        return super.customSettings();
    }

    // TODO: tbatie - 8/12/17 - Figure out why the SCR feature is running and taking so long

    // TODO: tbatie - 8/19/17 - Fix port once dynamic port is refactored for general testing
    public static final String GRAPHQL_ENDPOINT = "https://localhost:9993/admin/beta/graphql";
    ...
    public LdapConfigurationField createSampleLdapConfiguration(EnumValue<String> ldapUseCase) {
        ...
        LdapDirectorySettingsField dirSettings =
                new LdapDirectorySettingsField().baseUserDn(TEST_DN)
                        ...
                        .baseGroupDn(TEST_DN)
                        .useCase(ldapUseCase.value());

        if (ldapUseCase.value()
                .equals(LdapUseCase.ATTRIBUTE_STORE.value()) || ldapUseCase.value()
                .equals(LdapUseCase.AUTHENTICATION_AND_ATTRIBUTE_STORE.value())) {
