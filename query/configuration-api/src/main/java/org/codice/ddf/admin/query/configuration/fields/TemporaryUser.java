...
import org.codice.ddf.admin.query.configuration.services.impl.UserConfigurationImpl;
...
public class TemporaryUser extends BaseObjectField {
  ...
  private static final String DEFAULT_FIELD_NAME = "user";

  private static final String FIELD_TYPE_NAME = "TemporaryUser";

  private static final String DESCRIPTION = "A temporary user that is able to log into the system. This is for installation/testing purposes only and should not be used in production.";
  ...
  public TemporaryUserConfiguration toConfig() {
    return new UserConfigurationImpl(username.getValue(), password.getValue(), roles.getValue(), new HashMap<>());
  }
  ...
  public static class ListImpl extends BaseListField<TemporaryUser> {
    ...
    public ListImpl(List<TemporaryUserConfiguration> users) {
      this();
