...
public class CswSourceUtils {
    ...
    public static ProbeReport sendCswCapabilitiesRequest(String url, String username, String password) {
        ...
        if (statusCode == HTTP_OK && (contentType != null && CSW_MIME_TYPES.contains(contentType))) {
            return capabilitiesReport.addMessage(createCommonSourceConfigMsg(VERIFIED_CAPABILITIES)).probeResults(requestResults.probeResults());
        }
    ...
    /**
     * Attempts to create a configuration from the given url.
    ...
    public static ProbeReport getPreferredCswConfig(String url, String username, String password) {
        ProbeReport requestReport = new ProbeReport(sendCswCapabilitiesRequest(url, username, password).messages());
